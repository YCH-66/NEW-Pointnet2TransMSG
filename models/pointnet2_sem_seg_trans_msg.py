...
from models.pointnet2_utils import (
    PointNetSetAbstraction, PointNetSetAbstractionMsg, PointNetFeaturePropagation,
    square_distance, index_points
)
from losses_seg import combo_focal_dice
...
class LocalPointTransformer(nn.Module):
    def __init__(self, dim, num_heads=4, k=16, attn_drop=0.0, proj_drop=0.0):
        ...
        self.dim = dim; self.num_heads = num_heads; self.k = k
        d = dim // num_heads
        self.scale = d ** -0.5
    ...
    def forward(self, x, xyz):
        ...
        kv = self.kv(x)
        k, v = kv[:, :C, :], kv[:, C:, :]
        k = k.view(B, H, d, N); v = v.view(B, H, d, N)

        idx_exp = idx.unsqueeze(1).unsqueeze(2).expand(B, H, d, N, self.k)
        k_exp = k.unsqueeze(-1).expand(-1, -1, -1, -1, self.k)
        v_exp = v.unsqueeze(-1).expand(-1, -1, -1, -1, self.k)

        k_knn = k_exp.gather(dim=3, index=idx_exp)
        v_knn = v_exp.gather(dim=3, index=idx_exp)

        q_heads = q.transpose(2, 3)
        k_heads = k_knn.permute(0, 1, 3, 4, 2)

        attn_logits = (q_heads.unsqueeze(3) * k_heads).sum(-1) * self.scale
...
class PTBlock(nn.Module):
    def __init__(self, dim, depth=1, heads=4, k=16):
        super().__init__()
        self.blocks = nn.ModuleList([LocalPointTransformer(dim, heads, k) for _ in range(depth)])
    def forward(self, feat, xyz):
        for blk in self.blocks:
            feat = blk(feat, xyz)
        return feat

class PointNet2SemSegTransMSG(nn.Module):
    def __init__(self, num_classes: int, in_channel: int,
                 trans_k=16, depth_l1=1, depth_l2=2, depth_l3=2,
                 heads_l1=2, heads_l2=4, heads_l3=4):
        super().__init__()
        ...
        self.sa3 = PointNetSetAbstraction(
            ...
            in_channel=320, mlp=[128,128,256], group_all=False
        )
        ...
        self.pe_l1 = PosMLP(3, 224, 64)
        self.pe_l2 = PosMLP(3, 320, 96)
        self.pe_l3 = PosMLP(3, 256, 128)

        self.tr_l1 = PTBlock(dim=224, depth=depth_l1, heads=heads_l1, k=trans_k)
        self.tr_l2 = PTBlock(dim=320, depth=depth_l2, heads=heads_l2, k=trans_k)
        self.tr_l3 = PTBlock(dim=256, depth=depth_l3, heads=heads_l3, k=trans_k)

        self.fp4 = PointNetFeaturePropagation(in_channel=512 + 256, mlp=[256, 256])
        self.fp3 = PointNetFeaturePropagation(in_channel=256 + 320, mlp=[256, 256])
        self.fp2 = PointNetFeaturePropagation(in_channel=256 + 224, mlp=[256, 128])
    ...
    def forward(self, points):
        ...
        l1_points = self.tr_l1(l1_points + self.pe_l1(l1_xyz), l1_xyz)
        l2_points = self.tr_l2(l2_points + self.pe_l2(l2_xyz), l2_xyz)
        l3_points = self.tr_l3(l3_points + self.pe_l3(l3_xyz), l3_xyz)
...
def get_model(num_classes: int, in_channel: int = 3,
              trans_k=16, depth_l1=1, depth_l2=2, depth_l3=2,
              heads_l1=2, heads_l2=4, heads_l3=4):
    return PointNet2SemSegTransMSG(
        num_classes=num_classes, in_channel=in_channel,
        trans_k=trans_k, depth_l1=depth_l1, depth_l2=depth_l2, depth_l3=depth_l3,
        heads_l1=heads_l1, heads_l2=heads_l2, heads_l3=heads_l3
    )

def get_loss(pred, target, trans_feat=None, weight=None):
    return combo_focal_dice(pred, target, weight=weight, gamma=2.0, lambda_dice=0.2)